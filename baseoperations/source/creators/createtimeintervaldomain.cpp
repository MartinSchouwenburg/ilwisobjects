...
#include "createtimeintervaldomain.h"

using namespace Ilwis;
using namespace BaseOperations;

REGISTER_OPERATION(CreateTimeIntervalDomain)

CreateTimeIntervalDomain::CreateTimeIntervalDomain()
{

}

CreateTimeIntervalDomain::CreateTimeIntervalDomain(quint64 metaid, const Ilwis::OperationExpression &expr) : OperationImplementation(metaid, expr)
{

}

bool CreateTimeIntervalDomain::execute(ExecutionContext *ctx, SymbolTable &symTable)
{
	...
	intervaldomain->resourceRef().setExtendedType(itDATETIME);

	if (_parentdomain.isValid())
		intervaldomain->setParent(_parentdomain);
	for (int i = 0; i < _items.size(); i += 5) {
		TimeInterval rng(_items[i + 1], _items[i + 2], _resolution);
		auto *interval = new TimeIntervalItem(_items[i], rng);
...
Ilwis::OperationImplementation *CreateTimeIntervalDomain::create(quint64 metaid, const Ilwis::OperationExpression &expr)
{
	return new CreateTimeIntervalDomain(metaid, expr);
}

Ilwis::OperationImplementation::State CreateTimeIntervalDomain::prepare(ExecutionContext *ctx, const SymbolTable &st)
{
...
quint64 CreateTimeIntervalDomain::createMetadata()
{
	OperationResource resource({ "ilwis://operations/createtimeintervaldomain" });
	resource.setLongName("Create Time Interval Domain");
	resource.setSyntax("createtimeintervaldomain(intervaldefinition, resolution,description)");
	...
	resource.addInParameter(1, itSTRING, TR("Resolution"), TR("The resolution of numbers in the intervals used. All intervals have the same resolution "));

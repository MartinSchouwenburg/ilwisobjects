...
quint64 CreateThematicDomain::createMetadata()
{
    ...
    resource.addInParameter(0, itSTRING,TR("Item definition"), TR("A '|' separate list of item definition for the items of the domain, each item consist always of 3 parts, the last 2 parts maybe empty"));
    resource.addInParameter(1, itBOOL,TR("Strictness"), TR("Indicates how strict the relation between domain and parent is; true for all domains without parents"));

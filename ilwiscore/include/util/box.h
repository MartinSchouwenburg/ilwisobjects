...
public:
	...
	Box<PointType>& resize(double newwidth, double newheight, double newz=rUNDEF) {
		if (isNumericalUndef(newheight) || isNumericalUndef(newwidth) || newwidth < 0 ||newheight < 0)
			return *this;
		PointType center = center();
		PointType& pmin = this->min_corner();
		PointType& pmax = this->max_corner();
		double deltaX = newwidth / 2;
		double deltaY = newheight / 2;
		double deltaZ = 0;
		if (is3D())
			deltaZ = newz / 2;
		pmin = { center.x - deltaX, center.y - deltaY, center.z - deltaZ};
		pmax = { center.x + deltaX, center.y + deltaY, center.z + deltaZ};
		normalize();
